"""Dialog for the standalone Filter Engine."""
...
from kdl.engine.transaction_filter import create_filtered_workbook, suggest_filter_output
...
class _FilterWorker(QThread):
    ...
    def run(self):
        try:
            result = create_filtered_workbook(
                self.source_path,
                self.output_path,
                self.sheet_name,
            )
...
class TransactionFilterDialog(QDialog):
    """Independent Excel-to-Excel filtering workflow."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Filter Engine")
    ...
    def _build_ui(self):
        ...
        intro = QLabel(
            "Clean an IFMIS transaction export and create a separate filtered Excel workbook."
        )
        ...
        workbook_layout.addWidget(note)
        layout.addWidget(workbook_group)

        action_row = QHBoxLayout()
        action_row.addStretch()
        self._generate_button = QPushButton("Generate Filtered Workbook")
        self._generate_button.setMinimumWidth(220)
        ...
        result_group = QGroupBox("Filter Result")
        result_layout = QVBoxLayout(result_group)
        self._result_text = QTextEdit()
        self._result_text.setReadOnly(True)
        self._result_text.setMinimumHeight(105)
        self._result_text.setPlaceholderText(
            "Filtered row count and transaction totals will appear here..."
        )
    ...
    def _browse_source(self):
        ...
        self._output_path = suggest_filter_output(path)
    ...
    def _on_sheets_loaded(self, names: list):
        ...
        self._result_text.setPlainText(
            "Source workbook is ready. Select the transaction worksheet and "
            "generate the separate filtered workbook."
        )
    ...
    def _on_sheet_load_failed(self, message: str):
        ...
        QMessageBox.warning(self, "Filter Engine", message)
    ...
    def _choose_output(self):
        ...
        initial = self._output_path or (
            suggest_filter_output(source)
            if source
            else os.path.join(_default_browse_dir(), "Filtered_Data.xlsx")
        )
    ...
    def _generate(self):
        ...
        self._generate_button.setEnabled(False)
        self._generate_button.setText("Filtering...")
        self._open_button.setEnabled(False)
        self._result_text.setPlainText(
    ...
    def _on_completed(self, result):
        ...
        QMessageBox.information(
            self,
            "Filter Engine",
            "The separate filtered workbook was created successfully.",
        )

    def _on_failed(self, message: str):
        self._result_text.setPlainText(f"ERROR:\n{message}")
        self._reset_generate_button()
        QMessageBox.critical(self, "Filter Engine Error", message)

    def _reset_generate_button(self):
        self._generate_button.setText("Generate Filtered Workbook")
        self._update_generate_state()
