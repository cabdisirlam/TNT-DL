"""
Imprest Old Date AP Loader dialog.

Identical to ImprestSurrenderDialog but targets the imprest_old_date load mode
which sends Enter after every date field to dismiss Oracle IFMIS's
"date in prior period" dialog box.
"""
...
class _ImprestOldDateImportWorker(QThread):
...
class _ImprestOldDateReadWorker(QThread):
    ...
    def run(self):
        from kdl.engine.imprest_old_date_engine import read_invoice_rows

        self.rows, self.error = read_invoice_rows(self.filepath)


class _ImprestOldDateExportWorker(QThread):
    ...
    def run(self):
        from kdl.engine.imprest_old_date_engine import export_keystroke_sheet_to_workbook
...
class ImprestOldDateDialog(QDialog):
    """Convert a completed AP Imprest Surrender workbook into grid rows (old-date mode)."""

    load_into_grid = Signal(list)

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Imprest Old Date AP Loader")
    ...
    def _build_ui(self):
        ...
        intro = QLabel(
            "Old Date mode: Enter is sent after every date field to dismiss "
            "Oracle IFMIS’s prior-period date dialog automatically."
        )
        ...
        intro_row.addWidget(
            create_hint_button(
                "Use this loader when invoice or GL dates fall in a prior accounting period. "
                "Oracle IFMIS shows a confirmation dialog for past dates; this mode sends "
                "Enter to dismiss it before tabbing to the next field.",
                label="i",
            )
        )
    ...
    def _set_preview_rows(self, rows: list):
        from kdl.engine.imprest_old_date_engine import build_row_summary
    ...
    def _start_import(self, source_path: str, dest_path: str):
        ...
        worker = _ImprestOldDateImportWorker(source_path, dest_path)
        self._worker = worker
    ...
    def _on_import_finished(self):
        worker = self._worker
        if not isinstance(worker, _ImprestOldDateImportWorker):
            return
    ...
    def _start_read_rows(self, path: str):
        ...
        worker = _ImprestOldDateReadWorker(path)
        self._worker = worker
    ...
    def _on_read_rows_finished(self):
        worker = self._worker
        if not isinstance(worker, _ImprestOldDateReadWorker):
            return
    ...
    def _start_export(self, path: str):
        ...
        worker = _ImprestOldDateExportWorker(self._filepath, path, list(self._rows))
        self._worker = worker
    ...
    def _on_export_finished(self):
        worker = self._worker
        if not isinstance(worker, _ImprestOldDateExportWorker):
            return
    ...
    def _import_ifmis(self):
        ...
        dest, _ = QFileDialog.getSaveFileName(
            ...
            os.path.join(_default_dir(), "AP_Imprest_OldDate_Prefilled.xlsx"),
            "Excel Files (*.xlsx)",
        )
        if not dest:
            return
    ...
    def _load_into_grid(self):
        ...
        from kdl.engine.imprest_old_date_engine import COLUMNS
    ...
    def closeEvent(self, event):
        if self._worker is not None and self._worker.isRunning():
            QMessageBox.warning(
                ...
                "Wait for the current Imprest Old Date action to finish before closing this window.",
            )
            event.ignore()
            return
        self._release_worker()
